...
FROM_COLOR_CUSTOM = {250, 37, 50}                    -- Jeśli powyżej "custom", to tu ustawiasz RGB (0-1 lub 0-255)
...
SUBJECT_COLOR_TYPE = "custom"                         -- Kolor tematu: "white", "black", "custom"
SUBJECT_COLOR_CUSTOM = {255, 255, 255}                -- Własny kolor RGB tematu (gdy powyżej "custom")
...
SEPARATOR_COLOR_TYPE = "custom"        -- Kolor linii: "white", "black", "custom"
SEPARATOR_COLOR_CUSTOM = {255, 255, 255}  -- Własny kolor RGB (gdy "custom")
...
-- ====================================================================================
-- === OPTYMALIZACJA: NOWE KONTENERY CACHE (High Performance) ===
-- ====================================================================================
local AVATAR_SURFACE_CACHE = {}   -- Przechowuje gotowe, wycięte kółka/kwadraty avatarów
local BG_SURFACE_CACHE = nil      -- Przechowuje wyrysowane tło główne
local TRIM_CACHE = {}             -- Cache przyciętych tekstów (wersja Binary Search)
local split_emoji_cache = {}      -- Cache dla split_emoji
local width_cache = {}            -- Cache szerokości tekstów
local trim_cache = {}             -- Cache przycięć tekstów (stara wersja, dla kompatybilności)
local meta_width_cache = {}       -- Cache szerokości meta-linii
...
-- =========================================================================
-- split_emoji_cache jest zadeklarowane w sekcji OPTYMALIZACJA
...
-- =========================================================================
-- === OPTYMALIZACJA: Binarne przycinanie tekstu (Binary Search) ===
-- =========================================================================
-- Zastępuje starą funkcję trim_line_to_width_emoji
local function trim_line_to_width_emoji(cr, text, max_width, font_name, font_size, font_bold, font_italic)
    ...
    local cache_key = table.concat({text, max_width, font_name, font_size, tostring(font_bold), tostring(font_italic)}, "|")
    
    if TRIM_CACHE[cache_key] then return TRIM_CACHE[cache_key] end

    -- Ustawiamy font (ważne, jeśli funkcja wywołana bez wcześniejszego set_font)
    if font_name then 
        cairo_select_font_face(cr, font_name, font_italic and CAIRO_FONT_SLANT_ITALIC or CAIRO_FONT_SLANT_NORMAL, font_bold and CAIRO_FONT_WEIGHT_BOLD or CAIRO_FONT_WEIGHT_NORMAL)
        cairo_set_font_size(cr, font_size)
    end

    -- Szybki test całości - jeśli pasuje, zwracamy od razu
    cairo_text_extents(cr, text, GLOBAL_TEXT_EXTENTS)
    if GLOBAL_TEXT_EXTENTS.x_advance <= max_width then
    ...
    local ellipsis_width = GLOBAL_TEXT_EXTENTS.x_advance
    local target_width = max_width - ellipsis_width
    
    if target_width <= 0 then return ellipsis end

    -- Wyszukiwanie binarne (Binary Search) - dużo szybsze niż liniowe
    local low = 0
    local high = utf8_len(text)
    local best_idx = 0
    ...
    while low <= high do
        ...
        -- Mierzymy szerokość podciągu
        local w = text_width_with_emoji(cr, sub, font_name, font_size, font_bold, font_italic)
    ...
    local result = utf8_sub(text, 1, best_idx) .. ellipsis
...
-- Zmienne cache zadeklarowane wyżej w sekcji OPTYMALIZACJA
...
local function cached_trim_emoji(cr, text, maxw, font, size, bold, italic)
    -- Ta funkcja teraz korzysta z ulepszonej wersji binarnej (trim_line_to_width_emoji)
    return trim_line_to_width_emoji(cr, text, maxw, font, size, bold, italic)
end
-- [/PATCH]
...
-- =========================================================================
-- === OPTYMALIZACJA: ICON SURFACE CACHE (High Performance) ===
-- =========================================================================
local ICON_SURFACE_CACHE = {}

local function get_cached_icon_surface(cr, path, target_w, target_h, angle, mirror)
    local key = path .. "|" .. tostring(target_w) .. "|" .. tostring(target_h) .. "|" .. tostring(angle or 0) .. "|" .. tostring(mirror)
    
    if ICON_SURFACE_CACHE[key] then return ICON_SURFACE_CACHE[key] end

    local raw_image = get_png_surface(path)
    if not raw_image then return nil end

    local raw_w = cairo_image_surface_get_width(raw_image)
    local raw_h = cairo_image_surface_get_height(raw_image)

    local surface = cairo_surface_create_similar(cairo_get_target(cr), CAIRO_CONTENT_COLOR_ALPHA, target_w, target_h)
    local ctx = cairo_create(surface)

    cairo_translate(ctx, target_w / 2, target_h / 2)
    cairo_rotate(ctx, math.rad(angle or 0))

    local scale_x = target_w / raw_w
    local scale_y = target_h / raw_h
    ...
    cairo_set_source_surface(ctx, raw_image, -raw_w / 2, -raw_h / 2)
    cairo_paint(ctx)
    cairo_destroy(ctx)

    ICON_SURFACE_CACHE[key] = surface
    return surface
end

local function draw_png_rotated(cr, x, y, w, h, path, angle_deg, mirror_x)
    local surface = get_cached_icon_surface(cr, path, w, h, angle_deg, mirror_x)
    if surface then
...
local function trim_line_to_width(cr, text, max_width)
    -- Ta funkcja jest obecnie zastępowana przez wersję _emoji, ale dla bezpieczeństwa
    -- użyjemy nowej, szybkiej implementacji binarnej
    return trim_line_to_width_emoji(cr, text, max_width, nil, nil, nil, nil)
end
...
-- ====================================================================================
-- === OPTYMALIZACJA: CACHE TŁA GŁÓWNEGO (High Performance) ===
-- ====================================================================================
local function draw_main_background(cr)
    if not MAIN_BG_FILL_ENABLE and not MAIN_BG_BORDER_ENABLE then return end
    if conky_window == nil then return end

    -- Sprawdzamy, czy musimy przeliczyć tło (zmiana rozmiaru okna lub start)
    if BG_SURFACE_CACHE then
        local cw = cairo_image_surface_get_width(BG_SURFACE_CACHE)
        local ch = cairo_image_surface_get_height(BG_SURFACE_CACHE)
        if cw ~= conky_window.width or ch ~= conky_window.height then
            cairo_surface_destroy(BG_SURFACE_CACHE)
            BG_SURFACE_CACHE = nil
        end
    end

    -- Jeśli nie ma w cache, rysujemy (tylko RAZ)
    if not BG_SURFACE_CACHE then
        -- Tworzymy powierzchnię o rozmiarze okna
        BG_SURFACE_CACHE = cairo_surface_create_similar(cairo_get_target(cr), CAIRO_CONTENT_COLOR_ALPHA, conky_window.width, conky_window.height)
        local bg_cr = cairo_create(BG_SURFACE_CACHE)

        -- Tutaj oryginalna logika rysowania, ale na 'bg_cr' zamiast 'cr'
        ...
        local x, y = final_left, final_top
        ...
        cairo_move_to(bg_cr, x + radius, y)
        cairo_arc(bg_cr, x + w - radius, y + radius, radius, -math.pi/2, 0)
        cairo_arc(bg_cr, x + w - radius, y + h - radius, radius, 0, math.pi/2)
        cairo_arc(bg_cr, x + radius, y + h - radius, radius, math.pi/2, math.pi)
        cairo_arc(bg_cr, x + radius, y + radius, radius, math.pi, 1.5*math.pi)
        cairo_close_path(bg_cr)

        if MAIN_BG_FILL_ENABLE then
            local r, g, b = decode_rgb3(MAIN_BG_FILL_COLOR)
            cairo_set_source_rgba(bg_cr, r, g, b, clamp01(MAIN_BG_FILL_ALPHA or 0.5))
            cairo_fill_preserve(bg_cr)
        end

        if MAIN_BG_BORDER_ENABLE then
            local br, bg, bb = decode_rgb3(MAIN_BG_BORDER_COLOR)
            cairo_set_source_rgba(bg_cr, br, bg, bb, clamp01(MAIN_BG_BORDER_ALPHA or 0.2))
            cairo_set_line_width(bg_cr, MAIN_BG_BORDER_WIDTH or 2)
            cairo_stroke(bg_cr)
        end
        
        cairo_destroy(bg_cr) -- niszczymy tylko "pędzel", obraz zostaje w BG_SURFACE_CACHE
    end

    -- Rysowanie z Cache (Błyskawiczne)
    cairo_set_source_surface(cr, BG_SURFACE_CACHE, 0, 0)
    cairo_paint(cr)
end
...
-- =========================================================================
-- === OPTYMALIZACJA: CACHE POWIERZCHNI AWATARÓW (High Performance) ===
-- =========================================================================
local function get_cached_avatar_surface(cr, path, size, shape)
    local key = path .. "|" .. size .. "|" .. tostring(shape)
    
    if AVATAR_SURFACE_CACHE[key] then
        return AVATAR_SURFACE_CACHE[key]
    end

    -- Jeśli nie ma w cache, tworzymy
    local raw_image = get_avatar_image(path) -- Używa istniejącej funkcji ładowania PNG
    if not raw_image then return nil end
    
    -- Tworzymy nową powierzchnię idealnie pod rozmiar avatara
    local surface = cairo_surface_create_similar(cairo_get_target(cr), CAIRO_CONTENT_COLOR_ALPHA, size, size)
    local ctx = cairo_create(surface)

    -- Rysujemy kształt wycięcia na tej małej powierzchni
    if shape == "circle" then
        cairo_arc(ctx, size/2, size/2, size/2, 0, 2*math.pi)
    elseif shape == "square" then
        cairo_rectangle(ctx, 0, 0, size, size)
    else -- rounded
        local r = size * 0.25
        -- Rysowanie zaokrąglonego prostokąta ręcznie dla pewności
        cairo_new_sub_path(ctx)
        cairo_move_to(ctx, r, 0)
        cairo_line_to(ctx, size - r, 0)
        cairo_curve_to(ctx, size, 0, size, 0, size, r)
        cairo_line_to(ctx, size, size - r)
        cairo_curve_to(ctx, size, size, size, size, size - r, size)
        cairo_line_to(ctx, r, size)
        cairo_curve_to(ctx, 0, size, 0, size, 0, size - r)
        cairo_line_to(ctx, 0, r)
        cairo_curve_to(ctx, 0, 0, 0, 0, r, 0)
        cairo_close_path(ctx)
    end
    cairo_clip(ctx)

    -- Skalujemy i rysujemy obrazek
    local img_w = cairo_image_surface_get_width(raw_image)
    local img_h = cairo_image_surface_get_height(raw_image)
    local scale = math.max(size / img_w, size / img_h)
    local trans_x = (size - (img_w * scale)) / 2
    local trans_y = (size - (img_h * scale)) / 2
    
    cairo_translate(ctx, trans_x, trans_y)
    cairo_scale(ctx, scale, scale)
    cairo_set_source_surface(ctx, raw_image, 0, 0)
    cairo_paint(ctx)
    
-- Rysowanie ramki (opcjonalnie zapiekamy ramkę w obrazek)
    if DRAW_AVATAR_BORDER then
        ...
        if c then
            ...
            cairo_set_line_width(ctx, AVATAR_BORDER_WIDTH or 2.5)
            
            -- Rysujemy ramkę po obrysie
            if shape == "circle" then
                 cairo_arc(ctx, size/2, size/2, size/2 - (AVATAR_BORDER_WIDTH/2), 0, 2*math.pi)
                 cairo_stroke(ctx)
            elseif shape == "square" then
                 cairo_rectangle(ctx, AVATAR_BORDER_WIDTH/2, AVATAR_BORDER_WIDTH/2, size-AVATAR_BORDER_WIDTH, size-AVATAR_BORDER_WIDTH)
                 cairo_stroke(ctx)
            
            -- >>>>>> TO JEST TA CZĘŚĆ, KTÓRĄ MUSISZ DODAĆ RĘCZNIE: START
            elseif shape == "rounded" then
                local bw = AVATAR_BORDER_WIDTH or 2.5
                local half = bw / 2
                local r = size * 0.25
                
                cairo_new_sub_path(ctx)
                cairo_move_to(ctx, r, half)
                cairo_line_to(ctx, size - r, half)
                cairo_curve_to(ctx, size - half, half, size - half, half, size - half, r)
                cairo_line_to(ctx, size - half, size - r)
                cairo_curve_to(ctx, size - half, size - half, size - half, size - half, size - r, size - half)
                cairo_line_to(ctx, r, size - half)
                cairo_curve_to(ctx, half, size - half, half, size - half, half, size - r)
                cairo_line_to(ctx, half, r)
                cairo_curve_to(ctx, half, half, half, half, r, half)
                cairo_close_path(ctx)
                cairo_stroke(ctx)
            -- <<<<<< TO JEST TA CZĘŚĆ, KTÓRĄ MUSISZ DODAĆ RĘCZNIE: KONIEC
            end
        end
    end
    ...
    AVATAR_SURFACE_CACHE[key] = surface
    return surface
end
...
local function draw_avatar_local(cr, mail_from_raw, x, y, size)
    ...
    -- UŻYCIE CACHE: Pobieramy gotowy surface zamiast liczyć od nowa
    local surface = get_cached_avatar_surface(cr, img_path, size, AVATAR_SHAPE or "circle")
...
local function _conky_draw_mail_indicator_impl()
    ...
    if not are_tables_equal(current_uids, previously_known_uids) then
        ...
        -- === OPTYMALIZACJA: CZYSZCZENIE CACHE PRZY ZMIANIE LISTY MAILI ===
        -- Czyścimy, bo przyszły nowe maile i stare przycięcia/avatary mogą być niepotrzebne
        TRIM_CACHE = {} 
        width_cache = {}
        split_emoji_cache = {} 
        
        -- Avatary czyścimy tylko jeśli lista się mocno zmieniła, 
        -- ale dla bezpieczeństwa pamięci można niszczyć powierzchnie:
        for k, v in pairs(AVATAR_SURFACE_CACHE) do
            cairo_surface_destroy(v)
        end
