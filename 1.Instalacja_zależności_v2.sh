...
# - Auto-detekcja dystrybucji, instalacja zależności Conky/Lua/Python
...
case "$DISTRO" in
  linuxmint|"linux mint"|mint)
    ...
    if [[ "$MAJOR_VER" == "21" || "$MAJOR_VER" == "22" ]]; then
      REQUIRED_PACKAGES=(conky-all wget lua5.4 liblua5.4-dev fonts-noto-color-emoji yad jq)
    else
      REQUIRED_PACKAGES=(conky-all wget lua5.3 liblua5.3-dev fonts-noto-color-emoji yad jq)
    fi
    ;;
  ubuntu)
    PM="apt-get"; INSTALL="sudo $PM install -y"
    PREINSTALL_CMD="sudo apt-get update"
    REQUIRED_PACKAGES=(conky-all wget fonts-noto-color-emoji yad jq)
  ...
  fedora)
    ...
    REQUIRED_PACKAGES=(conky wget lua lua-devel "$EMOJI_PKG" yad jq)
    ;;
  arch*|manjaro*|garuda*|endeavouros|artix)
    PM="pacman"; INSTALL="sudo $PM -S --noconfirm --needed"
    PREINSTALL_CMD="sudo pacman -Sy"
    REQUIRED_PACKAGES=(conky wget lua noto-fonts-emoji yad jq)
    ;;
  opensuse*|suse*)
    ...
    REQUIRED_PACKAGES=(conky wget lua "$EMOJI_PKG" yad jq)
    ;;
  solus)
    PM="eopkg"; INSTALL="sudo $PM install -y"
    PREINSTALL_CMD="sudo eopkg update-repo"
    REQUIRED_PACKAGES=(conky wget lua font-noto-emoji yad jq)
    ;;
  nixos)
    zenity_info_or_exit "ℹ️ <b>NixOS wykryty.</b>\nZainstaluj ręcznie pakiety: conky, lua, wget, yad, jq oraz noto-fonts-emoji przez configuration.nix." 520
    exit 0
    ;;
  *)
	...
	if apt-cache policy lua5.4 2>/dev/null | grep -q 'Candidate:[[:space:]]\+[0-9]'; then
      ...
      REQUIRED_PACKAGES=(conky wget lua lua-devel google-noto-emoji-color-fonts yad jq)
      ...
      zenity_info_or_exit "❗ Nie rozpoznano menedżera pakietów.\nDodaj ręcznie wymagane pakiety: conky, lua, wget, yad, jq oraz czcionkę Noto Color Emoji." 560
      exit 0
    fi
    ;;
esac
...
if zenity --question --title="Sukces! 🎉" --text="<big><big>Skrypt wykonał swoje zadanie 😊</big></big>\nCzy chcesz teraz uruchomić kolejny skrypt <b>\"2.Podmiana_wartości_w_zmiennych.sh\"</b>, który automatycznie ustawi odpowiednie ścieżki bezwzględne dla zmiennych w plikach projektu?\n<span foreground='red'>Jest to konieczne do prawidłowego działania widgetu.</span>" --ok-label="Tak" --cancel-label="Nie"; then
    ...
    zenity_info_or_exit "<b>✅ Zakończono instalację.</b> Możesz teraz ręcznie uruchomić skrypt: <b>2.Podmiana_wartości_w_zmiennych.sh</b>"
fi
