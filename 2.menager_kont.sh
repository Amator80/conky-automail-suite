#!/bin/bash
# ===================================================================================
# UNIWERSALNY KONFIGURATOR KONT E-MAIL (WERSJA SECURITY PRO - FINAL COMPLETE)
# - [UI FIX] Poprawiony komunikat błędu składni (wymienia przecinki, klamry itp.)
# - [UI FIX] Logika pierwszego uruchomienia i zmiany hasła
# - [FIX] Usuwanie/Zmiana hasła wymaga autoryzacji
# - [FIX] DEEP SCAN + Twardy Reset
# ===================================================================================

# --- ŚCIEŻKI KONFIGURACYJNE ---
SCRIPT_DIR="$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")"

# 1. Konfiguracja (zostaje w projekcie - Portable)
CONFIG_PATH="$SCRIPT_DIR/config/config.json"
SELECTOR_FILE="$SCRIPT_DIR/config/active_account_selector"
ATTEMPT_FILE="$SCRIPT_DIR/config/.failed_attempts"

# 2. Klucz (wychodzi do bezpiecznego katalogu użytkownika - Security)
USER_CONFIG_DIR="$HOME/.config/conky-mail-secret-key"
mkdir -p "$USER_CONFIG_DIR"
SECRET_KEY="$USER_CONFIG_DIR/.secret_key"

# --- MAGIA WERYFIKACJI ---
CHALLENGE_TEXT="ACCESS_GRANTED_VERIFIED_V1"
DISABLED_MARKER="SECURITY_DISABLED_BY_USER"

# --- SPRAWDZENIE ZALEŻNOŚCI ---
if ! command -v jq &> /dev/null || \
   ! command -v yad &> /dev/null || \
   ! command -v xrandr &> /dev/null || \
   ! command -v openssl &> /dev/null || \
   ! command -v notify-send &> /dev/null; then
    yad --center --fixed --error \
        --text="<b>Błąd: Brakujące zależności!</b>\n\nUpewnij się, że zainstalowane są pakiety:\n'jq', 'yad', 'openssl', 'x11-xserver-utils', 'libnotify-bin'.\n\n<b>sudo apt install jq yad openssl x11-xserver-utils libnotify-bin</b>" \
        --width=800
    exit 1
fi

# ===================================================================================
# INTELIGENTNA WERYFIKACJA PLIKU KONFIGURACYJNEGO
# ===================================================================================

if [ ! -f "$CONFIG_PATH" ] || [ ! -s "$CONFIG_PATH" ]; then
    mkdir -p "$(dirname "$CONFIG_PATH")"
    echo '{"master_hash": null, "accounts": []}' > "$CONFIG_PATH"
    echo "0" > "$SELECTOR_FILE"
else
    # Sprawdzamy składnię JSON
    JQ_OUTPUT=$(jq empty "$CONFIG_PATH" 2>&1)
    
    if [ $? -ne 0 ]; then
        # Wykryto błąd składni - ZAKTUALIZOWANY OPIS
        yad --center --fixed --image="dialog-error" --width=650 \
            --title="BŁĄD SKŁADNI PLIKU JSON" \
            --text="<big><b>Wykryto uszkodzoną strukturę pliku config.json!</b></big>\n\nPrawdopodobnie plik był edytowany ręcznie i zawiera błędy składni\n(np. brak przecinka, cudzysłowu lub niedomknięta klamra).\n\n<b>Pierwszy napotkany błąd (od 'jq'):</b>\n<span face='Monospace' color='red'>$JQ_OUTPUT</span>\n\n<i>Uwaga: Parser zatrzymuje się na pierwszym błędzie. Po jego naprawieniu mogą pojawić się kolejne.</i>\n\nCo chcesz zrobić?" \
            --button="RESETUJ PLIK (Utrata danych):0" \
            --button="POPRAWIĘ RĘCZNIE (Wyjście):1"
        
        EXIT_CHOICE=$?
        
        if [ $EXIT_CHOICE -eq 0 ]; then
            mkdir -p "$(dirname "$CONFIG_PATH")"
            echo '{"master_hash": null, "accounts": []}' > "$CONFIG_PATH"
            rm -f "$ATTEMPT_FILE"
            echo "0" > "$SELECTOR_FILE"
            yad --info --center --text="Plik konfiguracyjny został zresetowany do ustawień fabrycznych."
        else
            exit 1
        fi
    fi
fi

# ===================================================================================
# SYSTEM HASŁA GŁÓWNEGO
# ===================================================================================

save_new_master_password() {
    local NEW_PASS="$1"
    local HASH_TO_SAVE
    
    if [ "$NEW_PASS" == "$DISABLED_MARKER" ]; then
        HASH_TO_SAVE="$DISABLED_MARKER"
    else
        # Szyfrujemy challenge nowym hasłem
        HASH_TO_SAVE=$(echo -n "$CHALLENGE_TEXT" | openssl enc -aes-256-cbc -salt -pbkdf2 -pass pass:"$NEW_PASS" -a -A)
    fi
    
    # Zapisujemy do JSON zachowując istniejące konta
    jq --arg h "$HASH_TO_SAVE" '{master_hash: $h, accounts: .accounts}' "$CONFIG_PATH" > "$CONFIG_PATH.tmp"
    
    if [ $? -eq 0 ] && [ -s "$CONFIG_PATH.tmp" ]; then
        mv "$CONFIG_PATH.tmp" "$CONFIG_PATH"
        return 0
    else
        rm -f "$CONFIG_PATH.tmp"
        return 1
    fi
}

change_password_gui() {
    local MODE="$1"
    local CURRENT_HASH
    CURRENT_HASH=$(jq -r '.master_hash // empty' "$CONFIG_PATH")
    
    local TITLE="Zarządzanie bezpieczeństwem"
    local TEXT_MSG=""
    local NEW_PASS_DATA
    local EXIT_CODE
    
    # --- LOGIKA INTERFEJSU (3 SCENARIUSZE) ---
    
    if [[ -z "$CURRENT_HASH" || "$CURRENT_HASH" == "null" ]]; then
        # 1. PIERWSZE URUCHOMIENIE
        TITLE="Tworzenie Hasła Głównego"
        TEXT_MSG="<b>Witaj w konfiguratorze!</b>\n\nTo Twoje pierwsze uruchomienie.\nZdefiniuj hasło główne, które będzie chronić Twoje konta e-mail."
        
        NEW_PASS_DATA=$(yad --form --center --fixed --width=400 --title="$TITLE" \
            --image="dialog-password" \
            --text="$TEXT_MSG" \
            --field="Nowe hasło::H" \
            --field="Powtórz hasło::H" \
            --button="USTAW HASŁO:0" --button="Anuluj:1")
        
        EXIT_CODE=$?

    elif [[ "$CURRENT_HASH" == "$DISABLED_MARKER" ]]; then
        # 2. TRYB OTWARTY
        TITLE="Włączanie zabezpieczeń"
        TEXT_MSG="<b>Tryb Otwarty jest aktywny.</b>\n\nWpisz hasło, aby włączyć zabezpieczenie programu."
        
        NEW_PASS_DATA=$(yad --form --center --fixed --width=400 --title="$TITLE" \
            --image="dialog-password" \
            --text="$TEXT_MSG" \
            --field="Nowe hasło::H" \
            --field="Powtórz hasło::H" \
            --button="WŁĄCZ OCHRONĘ:0" --button="Anuluj:1")
        
        EXIT_CODE=$?
        
    else
        # 3. STANDARDOWA ZMIANA
        TITLE="Zarządzanie hasłem"
        TEXT_MSG="<b>Zmiana Hasła Głównego</b>\n\nWpisz nowe hasło poniżej."
        
        if [ "$MODE" == "current_is_admin" ]; then
            TEXT_MSG="<b>Wykryto hasło domyślne (admin).</b>\nZaleca się ustawienie własnego, bezpiecznego hasła."
        fi
        
        NEW_PASS_DATA=$(yad --form --center --fixed --width=400 --title="$TITLE" \
            --image="dialog-password" \
            --text="$TEXT_MSG" \
            --field="Nowe hasło::H" \
            --field="Powtórz hasło::H" \
            --button="ZMIEŃ HASŁO:0" --button="USUŃ HASŁO (Tryb Otwarty):2" --button="Anuluj:1")
            
        EXIT_CODE=$?
    fi

    # --- OBSŁUGA AKCJI ---
    
    # 1. Anulowano (X lub przycisk Anuluj)
    if [ $EXIT_CODE -eq 1 ] || [ $EXIT_CODE -eq 252 ]; then 
        return 1
    fi

    # 2. Usuwanie hasła (Kod 2)
    if [ $EXIT_CODE -eq 2 ]; then
        local VERIFY_PASS
        VERIFY_PASS=$(yad --entry --center --fixed --width=400 --title="Weryfikacja tożsamości" \
            --window-icon="dialog-password" \
            --image="dialog-password" \
            --text="<b>Wymagana autoryzacja</b>\n\nAby wyłączyć zabezpieczenia (usunąć hasło),\nmusisz potwierdzić tożsamość wpisując <b>OBECNE</b> hasło:" \
            --hide-text \
            --button="Potwierdź:0" --button="Anuluj:1")
        
        if [ $? -ne 0 ]; then return 1; fi
        
        local CHECK_HASH
        CHECK_HASH=$(echo "$CURRENT_HASH" | openssl enc -d -aes-256-cbc -salt -pbkdf2 -pass pass:"$VERIFY_PASS" -a -A 2>/dev/null)
        
        if [[ "$CHECK_HASH" != "$CHALLENGE_TEXT" ]]; then
            yad --error --center --fixed --width=300 --text="<b>Błędne hasło!</b>\nNie można usunąć zabezpieczeń."
            change_password_gui "$MODE" # Powrót
            return $?
        fi

        if yad --question --center --fixed --width=400 --image="dialog-warning" \
            --title="Potwierdzenie" \
            --text="Hasło poprawne.\n\n<span weight='bold' color='red'>Czy na pewno chcesz usunąć hasło?</span>\n\nKażdy użytkownik komputera będzie miał pełny dostęp do konfiguracji."; then
            save_new_master_password "$DISABLED_MARKER"
            yad --info --center --text="Hasło zostało usunięte. Tryb otwarty aktywny."
            return 0
        else
            change_password_gui "$MODE" # Powrót
            return $?
        fi
    fi

    # 3. Zmiana/Ustawienie hasła (Kod 0)
    local P1=$(echo "$NEW_PASS_DATA" | cut -d'|' -f1)
    local P2=$(echo "$NEW_PASS_DATA" | cut -d'|' -f2)

    # Walidacja nowych haseł
    if [[ -z "$P1" ]]; then
        yad --error --center --text="Hasło nie może być puste!"
        change_password_gui "$MODE"
        return $?
    elif [[ "$P1" != "$P2" ]]; then
        yad --error --center --text="Hasła nie są identyczne!"
        change_password_gui "$MODE"
        return $?
    fi

    # Weryfikacja starego hasła (Autoryzacja przy zmianie)
    if [[ -n "$CURRENT_HASH" && "$CURRENT_HASH" != "null" && "$CURRENT_HASH" != "$DISABLED_MARKER" ]]; then
        local VERIFY_PASS_CHANGE
        VERIFY_PASS_CHANGE=$(yad --entry --center --fixed --width=400 --title="Potwierdź zmianę" \
            --window-icon="dialog-password" \
            --image="dialog-password" \
            --text="<b>Autoryzacja zmiany</b>\n\nAby zatwierdzić nowe hasło, podaj <b>STARE (OBECNE)</b> hasło:" \
            --hide-text \
            --button="Zatwierdź zmianę:0" --button="Anuluj:1")
        
        if [ $? -ne 0 ]; then return 1; fi

        local CHECK_HASH_CHANGE
        CHECK_HASH_CHANGE=$(echo "$CURRENT_HASH" | openssl enc -d -aes-256-cbc -salt -pbkdf2 -pass pass:"$VERIFY_PASS_CHANGE" -a -A 2>/dev/null)

        if [[ "$CHECK_HASH_CHANGE" != "$CHALLENGE_TEXT" ]]; then
            yad --error --center --fixed --width=300 --text="<b>Błędne stare hasło!</b>\nZmiana hasła została odrzucona."
            change_password_gui "$MODE"
            return $?
        fi
    fi

    # Zapisz
    if save_new_master_password "$P1"; then
        yad --info --center --text="Konfiguracja bezpieczeństwa zaktualizowana."
        return 0
    else
        yad --error --center --text="Błąd zapisu konfiguracji!"
        return 1
    fi
}

verify_access() {
    local CURRENT_BLOB
    local ACCOUNT_COUNT
    CURRENT_BLOB=$(jq -r '.master_hash // empty' "$CONFIG_PATH")
    ACCOUNT_COUNT=$(jq '.accounts | length' "$CONFIG_PATH")

    # =======================================================================
    # 1. DEEP SCAN (Kontrola WSZYSTKICH haseł)
    # =======================================================================
    
    local INTEGRITY_BREACH=false
    local BREACH_REPORT=""

    if (( ACCOUNT_COUNT > 0 )); then
        for ((i=0; i<ACCOUNT_COUNT; i++)); do
            local pass
            local login
            pass=$(jq -r ".accounts[$i].password" "$CONFIG_PATH")
            login=$(jq -r ".accounts[$i].login // \"Bez nazwy\"" "$CONFIG_PATH")
            
            if [[ "$pass" != "null" && -n "$pass" ]]; then
                if [[ "$pass" != U2FsdGVkX1* ]]; then
                    INTEGRITY_BREACH=true
                    BREACH_REPORT+="- Konto #$((i+1)) ($login): Hasło niezaszyfrowane\n"
                fi
            fi
        done
    fi

    if (( ACCOUNT_COUNT > 0 )); then
        local IS_VALID_MASTER=false
        if [[ "$CURRENT_BLOB" == U2FsdGVkX1* ]]; then IS_VALID_MASTER=true; fi
        
        if [[ "$CURRENT_BLOB" != "$DISABLED_MARKER" ]]; then
            if [[ -z "$CURRENT_BLOB" || "$CURRENT_BLOB" == "null" || "$IS_VALID_MASTER" == "false" ]]; then
                INTEGRITY_BREACH=true
                BREACH_REPORT="- GŁÓWNY KLUCZ (Master Hash): Uszkodzony lub sfałszowany\n$BREACH_REPORT"
            fi
        fi
    fi

    if [ "$INTEGRITY_BREACH" == "true" ]; then
        yad --center --fixed --error --title="NARUSZENIE INTEGRALNOŚCI (DEEP SCAN)" --width=600 \
            --image="dialog-error" \
            --text="<span color='red' size='large'><b>WYKRYTO PRÓBĘ FAŁSZOWANIA DANYCH!</b></span>\n\nMechanizm bezpieczeństwa wykrył następujące anomalie:\n\n<b>$BREACH_REPORT</b>\nKtoś edytował plik konfiguracyjny ręcznie wpisując niezaszyfrowane dane.\n\n<b>Procedura bezpieczeństwa: TWARDY RESET DANYCH.</b>"
        
        echo '{"master_hash": null, "accounts": []}' > "$CONFIG_PATH"
        rm -f "$ATTEMPT_FILE"
        echo "0" > "$SELECTOR_FILE"
        verify_access
        return $?
    fi

    # =======================================================================
    # 2. SPRAWDZENIE LEGALNEGO TRYBU OTWARTEGO
    # =======================================================================
    if [[ "$CURRENT_BLOB" == "$DISABLED_MARKER" ]]; then
        rm -f "$ATTEMPT_FILE"
        return 0
    fi

    # 3. PIERWSZE URUCHOMIENIE
    if [[ -z "$CURRENT_BLOB" || "$CURRENT_BLOB" == "null" ]]; then
        rm -f "$ATTEMPT_FILE"
        
        if yad --question --center --fixed --width=500 --image="dialog-question" \
            --title="Witaj w konfiguratorze" \
            --text="<big><b>Konfiguracja zabezpieczeń</b></big>\n\nCzy chcesz zabezpieczyć dostęp do tego programu hasłem głównym?\n\n(Zalecane jeśli komputer jest używany przez inne osoby)" \
            --button="TAK, ustaw hasło:0" --button="NIE, nie chcę hasła:1"; then
            
            if ! change_password_gui "enable"; then
                exit 1
            fi
        else
            save_new_master_password "$DISABLED_MARKER"
            yad --info --center --text="Program będzie działał w trybie otwartym.\nMożesz ustawić hasło później przyciskiem w menu."
        fi
        
        CURRENT_BLOB=$(jq -r '.master_hash // empty' "$CONFIG_PATH")
        if [[ "$CURRENT_BLOB" == "$DISABLED_MARKER" ]]; then return 0; fi
        verify_access
        return $?
    fi

    # 4. LOGOWANIE
    while true; do
        local FAILED_COUNT
        if [ -f "$ATTEMPT_FILE" ]; then
            FAILED_COUNT=$(cat "$ATTEMPT_FILE")
        else
            FAILED_COUNT=0
        fi
        
        if ! [[ "$FAILED_COUNT" =~ ^[0-9]+$ ]]; then FAILED_COUNT=0; fi
        local REMAINING=$((5 - FAILED_COUNT))

        if (( REMAINING <= 0 )); then
            yad --error --center --fixed --width=500 --image="dialog-error" \
                --title="BLOKADA BEZPIECZEŃSTWA" \
                --text="<span color='red' size='large'><b>PRZEKROCZONO LIMIT PRÓB!</b></span>\n\nLimit 5 prób został wyczerpany.\nZgodnie z polityką bezpieczeństwa, <b>wszystkie dane zostają usunięte.</b>"
            
            echo '{"master_hash": null, "accounts": []}' > "$CONFIG_PATH"
            rm -f "$ATTEMPT_FILE"
            echo "0" > "$SELECTOR_FILE"
            verify_access
            return $?
        fi

        local INPUT_PASS
        INPUT_PASS=$(yad --entry --center --fixed --width=450 --title="Logowanie" \
            --image="dialog-password" \
            --window-icon="dialog-password" \
            --text="<b>Podaj Hasło Główne</b>\nPozostało prób: <span color='red'><b>$REMAINING</b></span>" \
            --hide-text \
            --button="Zaloguj:0" --button="Anuluj:1" --button="RESET DANYCH:2")
        
        local EXIT_CODE=$?

        if [ $EXIT_CODE -ne 0 ] && [ $EXIT_CODE -ne 2 ]; then
            return 1
        fi

        if [ $EXIT_CODE -eq 2 ]; then
            if yad --question --center --fixed --width=500 --title="POTWIERDZENIE" \
                --image="dialog-warning" \
                --text="<span color='red' size='large'><b>CZY NA PEWNO?</b></span>\n\nTa operacja trwale usunie WSZYSTKIE skonfigurowane konta e-mail." \
                --button="TAK, USUŃ:0" --button="Anuluj:1"; then
                
                echo '{"master_hash": null, "accounts": []}' > "$CONFIG_PATH"
                rm -f "$ATTEMPT_FILE"
                echo "0" > "$SELECTOR_FILE"
                yad --info --center --text="Dane zostały wyczyszczone."
                verify_access
                return $?
            else
                continue
            fi
        fi

        local DECRYPTED_ATTEMPT
        DECRYPTED_ATTEMPT=$(echo "$CURRENT_BLOB" | openssl enc -d -aes-256-cbc -salt -pbkdf2 -pass pass:"$INPUT_PASS" -a -A 2>/dev/null)

        if [[ "$DECRYPTED_ATTEMPT" == "$CHALLENGE_TEXT" ]]; then
            rm -f "$ATTEMPT_FILE"
            return 0
        else
            FAILED_COUNT=$((FAILED_COUNT + 1))
            echo "$FAILED_COUNT" > "$ATTEMPT_FILE"
            yad --error --center --text="<b>Błędne hasło!</b>" --timeout=1
        fi
    done
}

# ===================================================================================
# FUNKCJE SZYFROWANIA I GUI
# ===================================================================================

ensure_key_exists() {
    mkdir -p "$(dirname "$SECRET_KEY")"
    if [ ! -f "$SECRET_KEY" ]; then
        openssl rand -base64 32 > "$SECRET_KEY"
        chmod 600 "$SECRET_KEY"
    fi
}

encrypt_pass() {
    local cleartext="$1"
    [[ -z "$cleartext" ]] && echo "" && return
    ensure_key_exists
    echo -n "$cleartext" | openssl enc -aes-256-cbc -salt -pbkdf2 -pass file:"$SECRET_KEY" -a -A
}

decrypt_pass() {
    local encrypted="$1"
    [[ -z "$encrypted" ]] && echo "" && return
    ensure_key_exists
    local decrypted
    decrypted=$(echo "$encrypted" | openssl enc -aes-256-cbc -d -salt -pbkdf2 -pass file:"$SECRET_KEY" -a -A 2>/dev/null)
    if [ $? -eq 0 ]; then echo "$decrypted"; else echo "$encrypted"; fi
}

SCREEN_GEOMETRY=$(xrandr | grep '*' | head -n 1 | awk '{print $1}')
SCREEN_WIDTH=$(echo "$SCREEN_GEOMETRY" | cut -d'x' -f1)
SCREEN_HEIGHT=$(echo "$SCREEN_GEOMETRY" | cut -d'x' -f2)

select_view() {
    mkdir -p "$(dirname "$SELECTOR_FILE")"
    local ENABLED_LOGINS=()
    while IFS= read -r login; do
        if [[ -n "$login" ]]; then
            ENABLED_LOGINS+=("$login")
        fi
    done < <(jq -r '.accounts[] | select(.enabled == true and .login != null and .login != "") | .login' "$CONFIG_PATH")

    local ACCOUNT_COUNT=${#ENABLED_LOGINS[@]}

    if (( ACCOUNT_COUNT == 0 )); then
        yad --center --fixed --info --text="<b>Informacja:</b> Nie znaleziono żadnych aktywnych kont z ustawionym loginem." --width=600
        return
    elif (( ACCOUNT_COUNT == 1 )); then
        echo "${ENABLED_LOGINS[0]}" > "$SELECTOR_FILE"
        yad --center --fixed --info --title="Automatyczny wybór" --text="Wykryto jedno aktywne konto:\n\n<b>${ENABLED_LOGINS[0]}</b>" --width=700
        return
    fi

    local MODE_CHOICE_RAW MODE_CHOICE
    MODE_CHOICE_RAW=$(yad --list --radiolist --center --title="Wybierz tryb wyświetlania" \
        --text="W jaki sposób chcesz wyświetlać konta?" \
        --width=600 --height=200 \
        --column="Wybór:RD" --column="Opcja" \
        --button="Wybierz:0" --button="Anuluj:1" \
        TRUE "Podsumowanie wszystkich aktywnych kont" \
        FALSE "Wybiorę konkretne konta") || return

    MODE_CHOICE=$(echo "$MODE_CHOICE_RAW" | cut -d'|' -f2)

    if [[ "$MODE_CHOICE" == "Podsumowanie wszystkich aktywnych kont" ]]; then
        echo "0" > "$SELECTOR_FILE"
        notify-send "Przełączono widok poczty" "Wyświetlane: <b>Wszystkie konta</b>"
        return
    fi

    local YAD_ROWS=()
    for login in "${ENABLED_LOGINS[@]}"; do
        YAD_ROWS+=("FALSE" "$login")
    done

    local CHOICE
    CHOICE=$(yad --list --checklist --center --title="Wybierz konta do wyświetlenia" \
        --text="Zaznacz konta (loginy), które mają być widoczne w Conky." \
        --width=640 --height=480 \
        --column="Wybierz:CHK" --column="Login / Nazwa:TEXT" \
        --multiple --print-column=2 --separator=$'\n' \
        --button="Zatwierdź:0" --button="Anuluj:1" \
        "${YAD_ROWS[@]}") || return

    local SELECTED_LOGINS=()
    while IFS= read -r line; do
        [[ -n "$line" ]] && SELECTED_LOGINS+=("$line")
    done <<< "$CHOICE"

    (( ${#SELECTED_LOGINS[@]} == 0 )) && yad --center --fixed --warning --text="Nie wybrano żadnego konta. Operacja anulowana." --width=500 && return

    if (( ${#SELECTED_LOGINS[@]} == ACCOUNT_COUNT )); then
        echo "0" > "$SELECTOR_FILE"
        notify-send "Przełączono widok poczty" "Wyświetlane: <b>Wszystkie konta (automatycznie)</b>"
        return
    fi

    printf '%s\n' "${SELECTED_LOGINS[@]}" > "$SELECTOR_FILE"
    local LOGINS_STR
    LOGINS_STR=$(printf '%s, ' "${SELECTED_LOGINS[@]}")
    LOGINS_STR="${LOGINS_STR%, }"
    notify-send "Przełączono widok poczty" "Wybrane konta: <b>$LOGINS_STR</b>"
}

edit_account() {
    local choice_string="$1"
    local CLEAN_CHOICE=$(echo "$choice_string" | grep "Konto #" | head -n 1)
    
    if [[ $(echo "$choice_string" | grep "Konto #" | wc -l) -gt 1 ]]; then
        yad --center --fixed --warning --text="Można edytować tylko jedno konto na raz." --width=500
        return
    fi

    local SLOT_NUM
    SLOT_NUM=$(echo "$CLEAN_CHOICE" | grep -o -E 'Konto #[0-9]+' | grep -o -E '[0-9]+')
    [[ -z "$SLOT_NUM" ]] && return
    local INDEX=$((SLOT_NUM - 1))

    local values
    mapfile -t values < <(jq -r --argjson index "$INDEX" '.accounts[$index] | [ .enabled, .name, .host, .port, .login, .password, .encryption, (.color // [255, 255, 255, 255] | .[0]), (.color // [255, 255, 255, 255] | .[1]), (.color // [255, 255, 255, 255] | .[2]), (.color // [255, 255, 255, 255] | .[3]) ] | .[]' "$CONFIG_PATH")

    local ENABLED=${values[0]}
    local NAME=$([ "${values[1]}" == "null" ] && echo "" || echo "${values[1]}")
    local HOST=$([ "${values[2]}" == "null" ] && echo "" || echo "${values[2]}")
    local PORT=$([ "${values[3]}" == "null" ] && echo "" || echo "${values[3]}")
    local LOGIN=$([ "${values[4]}" == "null" ] && echo "" || echo "${values[4]}")
    
    local RAW_PASS=$([ "${values[5]}" == "null" ] && echo "" || echo "${values[5]}")
    local PASSWORD=$(decrypt_pass "$RAW_PASS")
    
    local ENCRYPTION=$([ "${values[6]}" == "null" ] && echo "SSL" || echo "${values[6]}")
    local ALPHA=${values[7]}
    local RED=${values[8]}
    local GREEN=${values[9]}
    local BLUE=${values[10]}
    
    local COLOR_HEX
    COLOR_HEX=$(printf "#%02x%02x%02x" "$RED" "$GREEN" "$BLUE")
    local ALPHA_WITH_RANGE="$ALPHA!0..255..1"
    local CHECKED_STATE=$([ "$ENABLED" == "true" ] && echo "TRUE" || echo "FALSE")
    
    local ENCRYPTION_OPTIONS
    if [[ "$ENCRYPTION" == "STARTTLS" ]]; then
        ENCRYPTION_OPTIONS="^STARTTLS!SSL";
    else
        ENCRYPTION_OPTIONS="^SSL!STARTTLS";
    fi

    local EDIT_WIN_WIDTH=650
    local EDIT_WIN_HEIGHT=400
    local EDIT_POS_X=$(((SCREEN_WIDTH - EDIT_WIN_WIDTH) / 2))
    local EDIT_POS_Y=$(((SCREEN_HEIGHT - EDIT_WIN_HEIGHT) / 2))
    local SEP=$'\x1f'

    local EDIT_DATA
    EDIT_DATA=$(yad --form --title="Edycja konta #$SLOT_NUM" \
        --width="$EDIT_WIN_WIDTH" --height="$EDIT_WIN_HEIGHT" --geometry="+$EDIT_POS_X+$EDIT_POS_Y" \
        --separator="$SEP" \
        --field="Aktywne:CHK" \
        --field="Nazwa konta (opis):" \
        --field="Host IMAP:" \
        --field="Port:" \
        --field="Szyfrowanie:CB" \
        --field="Login (e-mail):" \
        --field="Hasło:H" \
        --field="Kolor nazwy:CLR" \
        --field="Przezroczystość (0-255):NUM" \
        --button="Zapisz:0" --button="Anuluj:1" \
        "$CHECKED_STATE" "$NAME" "$HOST" "$PORT" "$ENCRYPTION_OPTIONS" "$LOGIN" "$PASSWORD" "$COLOR_HEX" "$ALPHA_WITH_RANGE")
    
    local EDIT_EXIT_STATUS=$?
    if [ $EDIT_EXIT_STATUS -eq 0 ]; then
        local new_enabled new_name new_host new_port new_encryption new_login new_password new_color_hex new_alpha
        IFS="$SEP" read -r new_enabled new_name new_host new_port new_encryption new_login new_password new_color_hex new_alpha <<< "$EDIT_DATA"

        local MISSING_FIELDS=""

        # --- WALIDACJA DANYCH (BEZ OSTRZEŻENIA NA WSTĘPIE) ---
        if [[ "$new_enabled" == "TRUE" ]]; then
            if [[ -z "$new_host" ]]; then MISSING_FIELDS+="- Host IMAP\n"; fi
            if [[ -z "$new_port" ]]; then MISSING_FIELDS+="- Port\n"; fi
            if [[ -z "$new_login" ]]; then MISSING_FIELDS+="- Login (e-mail)\n"; fi
            if [[ -z "$new_password" ]]; then MISSING_FIELDS+="- Hasło\n"; fi

            if [[ -n "$MISSING_FIELDS" ]]; then
                new_enabled="FALSE" # Tylko wymuszamy status, komunikat będzie na końcu
            fi
        fi
        # ---------------------------------------------------

        local encrypted_password=$(encrypt_pass "$new_password")
        local hex_clean=${new_color_hex#\#}
        local r=$((16#${hex_clean:0:2}))
        local g=$((16#${hex_clean:2:2}))
        local b=$((16#${hex_clean:4:2}))
        local alpha=$(printf "%.0f" "$new_alpha")
        local new_enabled_json=$([ "$new_enabled" == "TRUE" ] && echo "true" || echo "false")

        jq --argjson idx "$INDEX" --argjson en "$new_enabled_json" --arg nm "$new_name" --arg hst "$new_host" \
           --arg prt "$new_port" --arg enc "$new_encryption" --arg log "$new_login" --arg pwd "$encrypted_password" \
           --argjson a "$alpha" --argjson r "$r" --argjson g "$g" --argjson b "$b" \
           '{master_hash: .master_hash, accounts: .accounts} | 
            .accounts[$idx] = {
                "enabled": $en,
                "name": (if $nm=="" then null else $nm end),
                "host": (if $hst=="" then null else $hst end),
                "port": (if $prt=="" then null else ($prt|tonumber) end),
                "login": (if $log=="" then null else $log end),
                "password": (if $pwd=="" then null else $pwd end),
                "encryption": $enc,
                "color": [$a, $r, $g, $b]
            }' "$CONFIG_PATH" > "$CONFIG_PATH.tmp" && mv "$CONFIG_PATH.tmp" "$CONFIG_PATH"
        
        # --- POJEDYNCZY KOMUNIKAT KOŃCOWY ---
        if [[ -n "$MISSING_FIELDS" ]]; then
            yad --center --fixed --info --button="OK:0" --width=600 --image="dialog-warning" --title="Zapisano z ostrzeżeniem" \
                --text="<b>Dane zostały zapisane, ale konto jest NIEAKTYWNE!</b>\n\nPowód: Próbowano aktywować konto bez wymaganych danych.\nBrakujące pola:\n<span color='red'><b>$MISSING_FIELDS</b></span>"
        else
            yad --center --fixed --info --button="OK:0" --width=600 --text="Zmiany dla konta #$SLOT_NUM zostały zapisane!"
        fi
    fi
}

add_account() {
    local ADD_WIN_WIDTH=650
    local ADD_WIN_HEIGHT=400
    local ADD_POS_X=$(((SCREEN_WIDTH - ADD_WIN_WIDTH) / 2))
    local ADD_POS_Y=$(((SCREEN_HEIGHT - ADD_WIN_HEIGHT) / 2))
    local SEP=$'\x1f'

    local ADD_DATA
    ADD_DATA=$(yad --form --title="Dodaj nowe konto" \
        --width="$ADD_WIN_WIDTH" --height="$ADD_WIN_HEIGHT" --geometry="+$ADD_POS_X+$ADD_POS_Y" \
        --separator="$SEP" \
        --field="Aktywne:CHK" \
        --field="Nazwa konta (opis):" \
        --field="Host IMAP:" \
        --field="Port:" \
        --field="Szyfrowanie:CB" \
        --field="Login (e-mail):" \
        --field="Hasło:H" \
        --field="Kolor nazwy:CLR" \
        --field="Przezroczystość (0-255):NUM" \
        --button="Dodaj:0" --button="Anuluj:1" \
        "FALSE" "" "" "" "^SSL!STARTTLS" "" "" "#ffffff" "255!0..255..1")
    
    local ADD_EXIT_STATUS=$?
    if [ $ADD_EXIT_STATUS -eq 0 ]; then
        local new_enabled new_name new_host new_port new_encryption new_login new_password new_color_hex new_alpha
        IFS="$SEP" read -r new_enabled new_name new_host new_port new_encryption new_login new_password new_color_hex new_alpha <<< "$ADD_DATA"

        local MISSING_FIELDS=""

        # --- WALIDACJA DANYCH (BEZ OSTRZEŻENIA NA WSTĘPIE) ---
        if [[ "$new_enabled" == "TRUE" ]]; then
            if [[ -z "$new_host" ]]; then MISSING_FIELDS+="- Host IMAP\n"; fi
            if [[ -z "$new_port" ]]; then MISSING_FIELDS+="- Port\n"; fi
            if [[ -z "$new_login" ]]; then MISSING_FIELDS+="- Login (e-mail)\n"; fi
            if [[ -z "$new_password" ]]; then MISSING_FIELDS+="- Hasło\n"; fi

            if [[ -n "$MISSING_FIELDS" ]]; then
                new_enabled="FALSE" # Tylko wymuszamy status, komunikat będzie na końcu
            fi
        fi
        # ---------------------------------------------------

        local encrypted_password=$(encrypt_pass "$new_password")
        local hex_clean=${new_color_hex#\#}
        local r=$((16#${hex_clean:0:2}))
        local g=$((16#${hex_clean:2:2}))
        local b=$((16#${hex_clean:4:2}))
        local alpha=$(printf "%.0f" "$new_alpha")
        local new_enabled_json=$([ "$new_enabled" == "TRUE" ] && echo "true" || echo "false")

        jq --argjson en "$new_enabled_json" --arg nm "$new_name" --arg hst "$new_host" \
           --arg prt "$new_port" --arg enc "$new_encryption" --arg log "$new_login" --arg pwd "$encrypted_password" \
           --argjson a "$alpha" --argjson r "$r" --argjson g "$g" --argjson b "$b" \
           '{master_hash: .master_hash, accounts: (.accounts + [{
                "enabled": $en,
                "name": (if $nm=="" then null else $nm end),
                "host": (if $hst=="" then null else $hst end),
                "port": (if $prt=="" then null else ($prt|tonumber) end),
                "login": (if $log=="" then null else $log end),
                "password": (if $pwd=="" then null else $pwd end),
                "encryption": $enc,
                "color": [$a, $r, $g, $b]
            }])}' "$CONFIG_PATH" > "$CONFIG_PATH.tmp" && mv "$CONFIG_PATH.tmp" "$CONFIG_PATH"
        
        # --- POJEDYNCZY KOMUNIKAT KOŃCOWY ---
        if [[ -n "$MISSING_FIELDS" ]]; then
            yad --center --fixed --info --button="OK:0" --width=500 --image="dialog-warning" --title="Zapisano z ostrzeżeniem" \
                --text="<b>Konto zostało dodane, ale jest NIEAKTYWNE!</b>\n\nPowód: Próbowano aktywować konto bez wymaganych danych.\nBrakujące pola:\n<span color='red'><b>$MISSING_FIELDS</b></span>"
        else
            yad --center --fixed --info --button="OK:0" --width=500 --text="Nowe konto zostało dodane!"
        fi
    fi
}

delete_account() {
    local all_selections="$1"
    if [[ -z "$all_selections" ]]; then
        yad --center --fixed --warning --text="Najpierw zaznacz konto lub konta, które chcesz usunąć." --width=600
        return
    fi

    local INDICES_TO_DELETE=()
    local LOGINS_TO_DELETE=()
    local IDENTIFIERS_FOR_PROMPT=()

    while IFS= read -r line; do
        local SLOT_NUM
        SLOT_NUM=$(echo "$line" | grep -o -E 'Konto #[0-9]+' | grep -o -E '[0-9]+')
        [[ -z "$SLOT_NUM" ]] && continue
        local INDEX=$((SLOT_NUM - 1))
        
        local login_to_delete name_to_delete account_identifier
        mapfile -t account_data < <(jq -r --argjson index "$INDEX" '.accounts[$index] | .login, .name' "$CONFIG_PATH")
        login_to_delete="${account_data[0]}"
        name_to_delete="${account_data[1]}"
        
        if [[ "$login_to_delete" != "null" && -n "$login_to_delete" ]]; then
            account_identifier="$login_to_delete"
            LOGINS_TO_DELETE+=("$login_to_delete")
        elif [[ "$name_to_delete" != "null" && -n "$name_to_delete" ]]; then
            account_identifier="$name_to_delete (konto bez loginu)"
        else
            account_identifier="Nieskonfigurowane konto #$SLOT_NUM"
        fi
        INDICES_TO_DELETE+=("$INDEX")
        IDENTIFIERS_FOR_PROMPT+=("$account_identifier")
    done <<< "$all_selections"

    if [ ${#INDICES_TO_DELETE[@]} -eq 0 ]; then
        return;
    fi

    local prompt_list
    prompt_list=$(printf '<b>%s</b>\n' "${IDENTIFIERS_FOR_PROMPT[@]}")
    yad --center --fixed --question --title="Potwierdzenie usunięcia" --width=700 \
        --text="<span size='large'>Czy na pewno chcesz trwale usunąć zaznaczone konta?</span>\n\n${prompt_list}\nTej operacji nie można cofnąć." \
        --button="Tak, usuń:0" --button="Nie, wróć:1"
    
    local DELETE_CONFIRM_STATUS=$?
    if [ $DELETE_CONFIRM_STATUS -eq 0 ]; then
        local sorted_indices
        sorted_indices=($(printf '%s\n' "${INDICES_TO_DELETE[@]}" | sort -nr))
        
        local temp_json
        temp_json=$(cat "$CONFIG_PATH")
        for index in "${sorted_indices[@]}"; do
            temp_json=$(echo "$temp_json" | jq --argjson index "$index" '{master_hash: .master_hash, accounts: del(.accounts[$index]).accounts}')
        done
        echo "$temp_json" > "$CONFIG_PATH.tmp" && mv "$CONFIG_PATH.tmp" "$CONFIG_PATH"

        local view_updated=false
        if [ -s "$SELECTOR_FILE" ]; then
            for login in "${LOGINS_TO_DELETE[@]}"; do
                if grep -q -x "$login" "$SELECTOR_FILE"; then
                    grep -v -x "$login" "$SELECTOR_FILE" > "$SELECTOR_FILE.tmp" && mv "$SELECTOR_FILE.tmp" "$SELECTOR_FILE"
                    view_updated=true
                fi
            done
            if [ ! -s "$SELECTOR_FILE" ]; then
                echo "0" > "$SELECTOR_FILE"
            fi
        fi
        
        local final_message="Następujące konta zostały usunięte:\n\n${prompt_list}"
        if [ "$view_updated" = true ]; then
            final_message+="\nWidok wyświetlania został zaktualizowany."
        fi
        yad --center --fixed --info --button="OK:0" --width=700 --text="$final_message"
    fi
}

move_account() {
    local all_selections="$1"
    local direction="$2"
    
    local CLEAN_SELECTION=$(echo "$all_selections" | grep "Konto #" | head -n 1)

    if [[ -z "$CLEAN_SELECTION" ]]; then
        yad --center --fixed --warning --text="Aby zmienić kolejność, musisz zaznaczyć dokładnie jedno konto." --width=600
        return
    fi

    local SLOT_NUM
    SLOT_NUM=$(echo "$CLEAN_SELECTION" | grep -o -E 'Konto #[0-9]+' | grep -o -E '[0-9]+')
    [[ -z "$SLOT_NUM" ]] && return
    local INDEX=$((SLOT_NUM - 1))
    
    local ACCOUNT_COUNT
    ACCOUNT_COUNT=$(jq '.accounts | length' "$CONFIG_PATH")
    
    local TARGET_INDEX
    if [[ "$direction" == "up" ]]; then
        if (( INDEX <= 0 )); then
            yad --center --fixed --info --text="Nie można przesunąć pierwszego elementu wyżej." --width=500
            return
        fi
        TARGET_INDEX=$((INDEX - 1))
    elif [[ "$direction" == "down" ]]; then
        if (( INDEX >= ACCOUNT_COUNT - 1 )); then
            yad --center --fixed --info --text="Nie można przesunąć ostatniego elementu niżej." --width=500
            return
        fi
        TARGET_INDEX=$((INDEX + 1))
    else
        return
    fi
    
    jq --argjson i "$INDEX" --argjson j "$TARGET_INDEX" \
       '{master_hash: .master_hash, accounts: (.accounts | (.[$i] as $tmp | .[$i] = .[$j] | .[$j] = $tmp))}' \
       "$CONFIG_PATH" > "$CONFIG_PATH.tmp" && mv "$CONFIG_PATH.tmp" "$CONFIG_PATH"
}


# ===================================================================================
# GŁÓWNA PĘTLA PROGRAMU
# ===================================================================================

if verify_access; then
    while true; do
        ACCOUNTS_INFO=()
        local ACCOUNT_COUNT
        ACCOUNT_COUNT=$(jq '.accounts | length' "$CONFIG_PATH")

        for i in $(seq 0 $((ACCOUNT_COUNT - 1))); do
            mapfile -t data < <(jq -r ".accounts[$i] | .enabled, .name, .login" "$CONFIG_PATH")
            enabled="${data[0]}"
            name="${data[1]}"
            login="${data[2]}"
            
            if [ "$enabled" == "true" ]; then
                status_text="<span color='green'><b>Aktywne</b></span>";
            else
                status_text="<span color='red'>Nieaktywne</span>";
            fi
            
            if [ "$login" != "null" ] && [ -n "$login" ]; then
                display_name="$login";
            elif [ "$name" != "null" ] && [ -n "$name" ]; then
                display_name="$name";
            else
                display_name="<span color='grey'>Puste miejsce</span>";
            fi
            
            ACCOUNTS_INFO+=("$status_text" "Konto #$((i + 1))" "$display_name")
        done
        
        WIN_WIDTH=800
        WIN_HEIGHT=700 
        POS_X=$(((SCREEN_WIDTH - WIN_WIDTH) / 2))
        POS_Y=$(((SCREEN_HEIGHT - WIN_HEIGHT) / 2))
        
        CHOICE_AND_SELECTION=$(yad --list \
            --title="Konfigurator kont e-mail" \
            --geometry="${WIN_WIDTH}x${WIN_HEIGHT}+$POS_X+$POS_Y" \
            --text="<b>Zarządzaj kontami e-mail.</b>\n- Kliknij dwukrotnie, aby <b>edytować</b> pojedyncze konto (HASŁA SĄ UKRYTE).\n- Zaznacz jedno konto i użyj przycisków, aby je <b>przesunąć</b>.\n- Aby <b>usunąć wiele kont naraz</b>, zaznacz je przytrzymując \Ctrl\ lub \Shift\.\n- Użyj przycisków na dole do wykonania pozostałych akcji." \
            --column="Stan:TEXT" --column="Slot" --column="Login / Nazwa:TEXT" \
            --print-column=2 \
            --multiple \
            --button="ZARZĄDZAJ HASŁEM:5" \
            --button="Dodaj konto:3" \
            --button="Usuń zaznaczone:4" \
            --button="Przesuń w górę:50" \
            --button="Przesuń w dół:60" \
            --button="Wybierz konta:2" \
            --button="Zamknij:1" \
            "${ACCOUNTS_INFO[@]}")
            
        EXIT_CODE=$?
        
        case $EXIT_CODE in
            0)
                edit_account "$CHOICE_AND_SELECTION"
                ;;
            1)
                break
                ;;
            2)
                select_view
                ;;
            3)
                add_account
                ;;
            4)
                delete_account "$CHOICE_AND_SELECTION"
                ;;
            5)
                # Wywołanie ręcznej zmiany hasła
                change_password_gui
                ;;
            50) 
                move_account "$CHOICE_AND_SELECTION" "up"
                ;;
            60) 
                move_account "$CHOICE_AND_SELECTION" "down"
                ;;
            *)
                break
                ;;
        esac
    done
fi

# --- BLOK STARTOWY ---
if yad --question --center --fixed \
    --title="Uruchamianie" \
    --text="<big><b>Czy chcesz uruchomić skrypt startowy Conky?</b></big>\n\n(3.START_skryptów_oraz_conky.sh)" \
    --button="Tak, uruchom:0" --button="Nie, zakończ:1" --width=600; then

    if [ -f "$SCRIPT_DIR/3.START_skryptów_oraz_conky.sh" ]; then
        bash "$SCRIPT_DIR/3.START_skryptów_oraz_conky.sh" &
    else
        yad --error --center --fixed --text="<b>Błąd:</b> Nie znaleziono pliku '3.START_skryptów_oraz_conky.sh'!" --width=500
    fi
fi

exit 0
