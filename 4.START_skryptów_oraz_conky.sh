...
#
#  Powyższa flaga --no-diag-log sprawia, że logowanie diagnostyczne do pliku
#  jest domyślnie WYŁĄCZONE (co jest zalecane do normalnego użytku).
#
#  Aby WŁĄCZYĆ zapisywanie logów (np. do debugowania), znajdź w powyższym
#  bloku linię "--no-diag-log \" i dodaj na jej początku znak '#'.
# ==============================================================================
    ...
    if command -v zenity >/dev/null 2>&1; then
        ...
        if [ $? -eq 0 ]; then
            ...
            zenity --question \
                ...
                --text="Czy chcesz ponownie uruchomić skrypt 4.START_skryptów_oraz_conky.sh?"
            if [ $? -eq 0 ]; then
